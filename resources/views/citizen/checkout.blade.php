...
<div class="max-w-6xl mx-auto sm:py-8 selection:bg-blue-600 selection:text-white font-sans">
    <section class="rounded-2xl border border-slate-200 bg-white shadow-sm p-5 sm:p-8 lg:p-12 transition-all duration-300">
        ...
        <form method="POST" action="{{ route('portal.submit-checkout') }}" class="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-14">
            @csrf
                    ...
                    @if(isset($selectedService))
                        <div class="inline-flex items-center gap-2 rounded-lg bg-slate-50 border border-slate-200 px-3 py-2 text-xs text-slate-600 font-medium mt-2 w-full sm:w-auto">
                            ...
                            <span class="truncate">
                                ...
                                <span class="text-slate-900 font-bold ml-1">{{ $ne ? $selectedService->name_ne : $selectedService->name_en }}</span>
                            </span>
                        </div>
                    @endif
                </div>
                    ...
                    <div id="failure-section" class="hidden rounded-xl border border-slate-200 bg-slate-50/50 p-4 sm:p-5 transition-all duration-300">
                        ...
                        <div id="bribe-warning" class="hidden mt-4 rounded-xl border border-red-200 bg-red-50/60 p-4 text-red-900 border-l-4 border-l-red-600 animate-fadeIn shadow-sm">
                            <div class="flex items-start gap-3">
                                ...
                                <div>
                                    <p class="text-[11px] font-extrabold text-red-700 uppercase tracking-widest">{{ $ne ? 'अख्तियार/सदाचार अनुगमन सतर्कता' : 'Oversight Compliance Protocol' }}</p>
                                    <p class="mt-1 text-xs leading-relaxed text-red-800 font-medium">{{ $ne ? 'यो विवरण administrative निगरानी र सुशासन अडिटका लागि गृह मन्त्रालयको विशेष सदाचार शाखामा प्रत्यक्ष दर्ता हुनेछ।' : 'This feedback token is securely queued directly into internal anti-corruption compliance matrices.' }}</p>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            <div class="lg:col-span-6 flex flex-col justify-between space-y-6">
                ...
                <div class="space-y-6">
                    ...
                    <div class="space-y-2">
                        ...
                        <textarea name="comments" rows="4" class="w-full rounded-xl border border-slate-200 bg-slate-50/30 px-4 py-3.5 text-xs sm:text-sm font-normal focus:bg-white focus:border-blue-600 focus:ring-2 focus:ring-blue-600/20 focus:outline-none transition-all placeholder-slate-400 shadow-sm" placeholder="{{ $ne ? 'कार्यालयमा भोग्नुभएका विशिष्ट समस्या, ढिलासुस्ती वा सुधारका क्षेत्रहरू उल्लेख गर्नुहोस्...' : 'Specify parameter anomalies, structural counter delays or infrastructural optimization advice...' }}"></textarea>
                    </div>
                    ...
                    <div class="space-y-4">
                        <label class="flex items-center justify-between cursor-pointer p-4 rounded-xl bg-slate-50/80 border border-slate-200 hover:bg-slate-100 hover:border-slate-300 transition-all select-none shadow-sm group">
                            <div class="flex items-start gap-3.5 pr-4">
                                ...
                                <div class="text-left">
                                    <span class="block text-xs sm:text-sm font-extrabold text-slate-800 leading-none">{{ $ne ? 'पहिचान पूर्ण गोप्य राख्नुहोस्' : 'Strict Anonymity Routing' }}</span>
                                    <span class="block text-[10px] sm:text-xs text-slate-500 font-medium mt-1.5 leading-relaxed">{{ $ne ? 'तपाईंको नाम र फोन नम्बर डेटाबेसमा इन्क्रिप्ट गरिनेछ।' : 'Biometric profile linkage and metadata identifiers are entirely scrubbed.' }}</span>
                                </div>
                            </div>
                        ...
                        <div id="identity-fields" class="grid grid-cols-1 sm:grid-cols-2 gap-4 border-t border-slate-100 pt-4 transition-all duration-300">
                            <div>
                                <label class="block text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1.5">{{ $ne ? 'पूरा नाम' : 'Full Name' }}</label>
                            ...
                            <div>
                                <label class="block text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1.5">{{ $ne ? 'मोबाइल नम्बर' : 'Mobile Number' }}</label>
                ...
                <div class="flex flex-col-reverse sm:flex-row items-stretch sm:items-center gap-3 pt-4 sm:pt-2 border-t sm:border-t-0 border-slate-100 mt-6 sm:mt-0">
                    <a href="{{ route('portal.active-guide') }}" class="inline-flex items-center justify-center rounded-xl bg-slate-50 hover:bg-slate-100 border border-slate-200 px-5 py-3.5 sm:py-3 text-xs sm:text-sm font-bold text-slate-600 transition-all shadow-sm sm:shrink-0 text-center">
